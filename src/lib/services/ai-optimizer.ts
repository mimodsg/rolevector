...
export async function optimizeApplication({
  ...
  const optimizedCvJson: MasterCv = {
    ...
    work_experience: [...masterCv.work_experience]
      ...
      .sort((a, b) => relevanceScore(experienceText(b), relevantTerms) - relevanceScore(experienceText(a), relevantTerms)),
    projects: [...masterCv.projects].sort(
      (a, b) => relevanceScore(projectText(b), relevantTerms) - relevanceScore(projectText(a), relevantTerms)
    ),
    hidden_context: {
      additional_experience: masterCv.hidden_context.additional_experience,
      keywords: sortByRelevance(masterCv.hidden_context.keywords, relevantSkillSet)
    }
  };
  ...
  return {
    ...
    metadata: {
      ...
      notes: [
        ...
        `Matched ${relevantTerms.length} job terms already present in the Master CV source.`,
